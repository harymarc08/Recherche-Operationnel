...
interface EdgeModalProps {
  ...
  onClose: () => void;
  onSave: (source: string, target: string, weight: number, oldSource?: string, oldTarget?: string) => void;
  nodes: Node[];
}

const EdgeModal: React.FC<EdgeModalProps> = ({ isOpen, edge, onClose, onSave, nodes }) => {
        ...
        <div className="space-y-4">
          <div>
            ...
            <select 
              ...
              className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all"
            >
          ...
          <div>
            <label className="block mb-1 font-medium text-gray-700">Poids:</label>
            <input 
              type="number" 
              value={weight} 
...
const NodeModal: React.FC<NodeModalProps> = ({ isOpen, node, onClose, onSave }) => {
    ...
    <div className="fixed inset-0 bg-sky-900/60 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-xl shadow-xl w-3/4 max-w-2xl max-h-3/4 overflow-auto">
        ...
        <div className="space-y-4 text-gray-700">
          <div>
            <h3 className="font-semibold text-sky-700">Qu'est-ce que l'algorithme de Dijkstra modifié?</h3>
            <p>Cette application utilise une version modifiée de l'algorithme de Dijkstra pour trouver le chemin de poids maximal entre deux nœuds d'un graphe orienté et pondéré.</p>
          </div>
          <div>
            <h3 className="font-semibold text-sky-700">Comment utiliser cette application?</h3>
            <ol className="list-decimal pl-5 space-y-2">
              ...
              <li>Cliquez sur "Calculer" pour trouver le chemin de poids maximal</li>
...
export default function ModifiableGraphPage() {
  ...
  useEffect(() => {
    ...
    const updatedEdges = edges.map(edge => {
      if (path && path.includes(edge.source) && path.includes(edge.target) && 
  ...
  const edgesToGraph = (edges: Edge[]): GraphData => {
    ...
    edges.forEach(edge => {
      ...
      newGraph[edge.source][edge.target] = Number(edge.data?.weight || edge.label || 1);
    });
    return newGraph;
  };
  ...
  const handleSaveEdge = (source: string, target: string, weight: number, oldSource?: string, oldTarget?: string) => {
    if (oldSource && oldTarget) {
      ...
      if (oldSource === source && oldTarget === target) {
        ...
        setEdges(prev => 
          prev.map(edge => 
            edge.source === source && edge.target === target 
              ? { ...edge, label: weight.toString(), data: { ...edge.data, weight } } 
              : edge
          )
        );
      } else {
        ...
        setEdges(prev => [
          ...prev.filter(edge => !(edge.source === oldSource && edge.target === oldTarget)),
          {
            id: `${source}-${target}`,
            source,
            target,
    ...
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-xl shadow-md p-6 mb-6">
        <div className="flex justify-between items-center mb-4 border-b pb-3">
          <h1 className="text-2xl font-bold text-sky-800">Algorithme de Dijkstra - Chemin Maximal</h1>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-emerald-50 p-4 rounded-lg shadow-sm">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              >
                {nodes.map(node => (
                  <option key={`start-${node.id}`} value={node.id}>Départ: {node.id}</option>
                ))}
              </select>
              <select 
                value={endNode} 
              ...
              >
                {nodes.map(node => (
                  <option key={`end-${node.id}`} value={node.id}>Arrivée: {node.id}</option>
                ))}
              </select>
              <button 
          ...
          <div className="mb-6 p-5 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-emerald-200 shadow-sm">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-white p-3 rounded-lg shadow-sm border border-emerald-100">
                <p className="text-emerald-800 font-medium mb-1">Chemin maximal:</p>
                <div className="flex items-center flex-wrap gap-2">
                  {path.map((node, index) => (
                    <React.Fragment key={node}>
                      <span className="bg-emerald-100 text-emerald-800 font-bold py-1 px-3 rounded-lg">{node}</span>
                      {index < path.length - 1 && (
              ...
              <div className="bg-white p-3 rounded-lg shadow-sm border border-emerald-100">
                <p className="text-emerald-800 font-medium mb-1">Distance maximale:</p>
                <div className="flex items-center">
                  <span className="text-3xl font-bold text-emerald-600">{maxDistances[endNode]}</span>
        ...
        <div style={{ height: 500 }} className="rounded-lg overflow-hidden border border-gray-200 shadow-inner mb-6 bg-gray-50">
          <ReactFlowProvider>
            <ReactFlow
              nodes={nodes}
              edges={edges}
            ...
            >
              <Panel position="top-right" className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                <p className="text-gray-700 text-sm">
                  <span className="font-medium">Astuce:</span> Cliquez sur un nœud ou un arc pour le modifier
                </p>
              </Panel>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-sky-50 p-4 rounded-lg shadow-sm border border-sky-100">
            ...
            <div className="overflow-auto max-h-64 bg-white rounded-lg border border-sky-100">
              {nodes.length === 0 ? (
                <p className="p-3 text-gray-500 text-center">Aucun nœud disponible</p>
              ) : (
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-sky-50">
                    <tr>
                      <th className="px-4 py-2 text-left text-xs font-medium text-sky-800 uppercase tracking-wider">ID</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-sky-800 uppercase tracking-wider">Position</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-sky-800 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {nodes.map(node => (
                      <tr key={node.id} className="hover:bg-sky-50 transition-colors">
                        <td className="px-4 py-2 whitespace-nowrap font-medium text-sky-700">{node.id}</td>
          ...
          <div className="bg-indigo-50 p-4 rounded-lg shadow-sm border border-indigo-100">
            ...
            <div className="overflow-auto max-h-64 bg-white rounded-lg border border-indigo-100">
              {edges.length === 0 ? (
                <p className="p-3 text-gray-500 text-center">Aucun arc disponible</p>
              ) : (
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-indigo-50">
                    <tr>
                      <th className="px-4 py-2 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Source</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Destination</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Poids</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-indigo-800 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {edges.map(edge => (
                      <tr key={edge.id} className="hover:bg-indigo-50 transition-colors">
                        <td className="px-4 py-2 whitespace-nowrap font-medium text-indigo-700">{edge.source}</td>
                        <td className="px-4 py-2 whitespace-nowrap font-medium text-indigo-700">{edge.target}</td>
      ...
      <footer className="text-center text-gray-500 mt-8 pb-4">
        <p>Algorithme de Dijkstra modifié pour trouver le chemin maximal</p>
        <p className="text-sm mt-1">© {new Date().getFullYear()} - Interface améliorée</p>
      </footer>
